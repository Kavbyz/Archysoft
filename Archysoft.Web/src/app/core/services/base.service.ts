...
export class BaseService {
  ...
  get<T>(path:string, filter:BaseFilter):Observable<ApiResponse<T>>{
    return this.httpClient.get<ApiResponse<T>>(path, {
      params:new HttpParams().set('search', filter.search).set('orderBy', filter.orderBy).set('pageIndex', filter.pageIndex.toString()).set('pageSize', filter.pageSize.toString())
    });
  }
